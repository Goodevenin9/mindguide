...
def generate_response(user_input, school_name):
    ...
    for topic, responses in RESPONSE_TEMPLATES.items():
        if any(w in user_input for w in [topic, topic[:2]]):
            if school_name in responses:
                return responses[school_name]
    # 尝试关键词匹配
    for topic, responses in RESPONSE_TEMPLATES.items():
        keywords = {
            "焦虑": ["慌", "睡不着", "考试", "面试", "未来", "担心"],
            "压力": ["累", "忙", "崩溃", "太多", "喘不过气"],
            "人际关系": ["朋友", "吵架", "同学", "父母", "老师", "沟通", "社交"],
            "自我成长": ["改变", "提升", "进步", "目标", "迷茫", "方向"],
            "情绪低落": ["没劲", "不想", "难受", "哭", "没意思", "累"]
        }
        if topic in keywords:
            if any(w in user_input for w in keywords[topic]):
                if school_name in responses:
                    return responses[school_name]
    # 兜底回复
    return FALLBACK_RESPONSES.get(school_name, FALLBACK_RESPONSES["人本主义"])
